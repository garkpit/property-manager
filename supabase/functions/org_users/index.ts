...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // get a list of all users in the org
    console.log("calling rpc get_org_users with org_id", id);
    const { data: usersData, error: usersError } = await supabase
      .rpc("get_org_users", { org_id: id });
    console.log("get_org_users data", usersData);
    console.log("get_org_users error", usersError);

    if (usersError) {
      throw new Error(`Failed to get org users: ${usersError.message}`);
    }
