...
CREATE OR REPLACE FUNCTION "public"."delete_org"("org_id" "uuid")
    RETURNS "void"
    LANGUAGE "plpgsql"
    SECURITY DEFINER
    AS $$
...
CREATE POLICY "User must belong to org" ON "public"."contacts" TO "authenticated"
    USING (((
        SELECT
            "public"."get_org_role"("contacts"."orgid") AS "get_org_role") IS NOT NULL))
            WITH CHECK (((
                SELECT
                    "public"."get_org_role"("contacts"."orgid") AS "get_org_role") IS NOT NULL));
...
CREATE POLICY "org owners can create invites" ON "public"."orgs_invites"
    FOR INSERT TO "authenticated"
        WITH CHECK ((((
            SELECT
                "public"."get_org_role"("orgs_invites"."orgid") AS "get_org_role") = 'Owner'::"text") AND ("owner" =(
                    SELECT
                        "auth"."uid"() AS "uid"))));

CREATE POLICY "org owners can updated policies they created" ON "public"."orgs_invites"
...
CREATE POLICY "owner or invitee can delete an invite" ON "public"."orgs_invites"
...
CREATE POLICY "owner or invitee can view invite" ON "public"."orgs_invites"
...
CREATE POLICY "users can view orgs they belong to" ON "public"."orgs"
    FOR SELECT
        USING (("id" IN (
            ...
            FROM
                "public"."orgs_users"
            WHERE ("orgs_users"."userid" =(
                SELECT
                    "auth"."uid"() AS "uid")))));

CREATE POLICY "users can view their own records" ON "public"."orgs_users"
    FOR SELECT
